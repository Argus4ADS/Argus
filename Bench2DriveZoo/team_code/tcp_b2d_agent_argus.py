...
class TCPAgentMRP(TCPAgent):
    def setup(self, path_to_conf_file):
        super().setup(path_to_conf_file)
        self.track = autonomous_agent.Track.MAP
        self.mrconfig = mrconfig()
        self.last_lidar = None
        self.last_ego_transform = None
        self.tmp_visu = True
  
        # KBM
        self.ego_model = KinematicBicycleModel(self.mrconfig)
        self.vehicle_model = KinematicBicycleModel(self.mrconfig)
    ...
    def _init(self):
        ...
        obs_config = {
            'width_in_pixels': self.mrconfig.lidar_resolution_width,  # 256
            'pixels_ev_to_bottom': self.mrconfig.lidar_resolution_height / 2.0,  # 256/2 = 128
            'pixels_per_meter': self.mrconfig.pixels_per_meter_collection,  # 2.0
            'history_idx': [-1],
            'scale_bbox': True,
            'scale_mask_col': 1.0,
        ...
        self.ss_bev_manager = ObsManager(obs_config, self.mrconfig)
        ...
        self._waypoint_planner = RoutePlannerMR(self.mrconfig)
        ...
        self._longitudinal_controller = LongitudinalLinearRegressionController(self.mrconfig)
        self._turn_controller = LateralPIDController(self.mrconfig)
    ...
    def sensors(self):
        ...
        if self.save_path is not None:
            result.append({
                ...
                'x': self.mrconfig.camera_pos[0],
                'y': self.mrconfig.camera_pos[1],
                'z': self.mrconfig.camera_pos[2],
                'roll': self.mrconfig.camera_rot_0[0],
                'pitch': self.mrconfig.camera_rot_0[1],
                'yaw': self.mrconfig.camera_rot_0[2],
                'width': self.mrconfig.camera_width,
                'height': self.mrconfig.camera_height,
                'fov': self.mrconfig.camera_fov,
                'id': 'rgb1'
            })
            result.append({
                'type': 'sensor.lidar.ray_cast',
                'x': self.mrconfig.lidar_pos[0],
                'y': self.mrconfig.lidar_pos[1],
                'z': self.mrconfig.lidar_pos[2],
                'roll': self.mrconfig.lidar_rot[0],
                'pitch': self.mrconfig.lidar_rot[1],
                'yaw': self.mrconfig.lidar_rot[2],
                'rotation_frequency': self.mrconfig.lidar_rotation_frequency,
                'points_per_second': self.mrconfig.lidar_points_per_second,
                'id': 'lidar'
            })
    ...
    def _evaluate_hazards(self, monitor_data):
        ...
        try:
            ...
            if collision_time is not None and (collision_time <= previous_time or collision_time <= self.mrconfig.dangerous_time):
        ...
        try:
            if ego_speed < self.mrconfig.STOP_THRESHOLD_SPEED:
    ...
    def run_step(self, input_data, timestamp):
        ...
        input_data['lidar'] = t_u.lidar_to_ego_coordinate(self.mrconfig, input_data['lidar'])

        ads_control = super().run_step(input_data, timestamp)
        
        if self.step < self.mrconfig.seq_len:
            return ads_control
        
        # BEV Grid Map Generation
        lidar_bev_image = self.lidar_to_histogram_features(self.tick_data['lidar'])
        theta_to_lidar = self.tick_data['compass']
        local_command_xy = [
            self.tick_data['target_point'][0].item(),
            self.tick_data['target_point'][1].item()
        ]

        bounding_boxes, draw_bounding_boxes = t_u.parse_bounding_boxes(self.mrconfig, self.tick_data['bounding_boxes'])
    ...
    def save(self, tick_data):
        ...
        frame = self.step // 10
        Image.fromarray(tick_data["privileged_bev"]).save(self.save_path / 'privilege_bev' / ('%04d.png' % frame))
    ...
    def get_bounding_boxes(self, lidar=None):
        ...
        for vehicle in vehicle_list:
            if vehicle.get_location().distance(self._vehicle.get_location()) < self.mrconfig.bb_save_radius:
        ...
        for walker in walkers:
            if walker.get_location().distance(self._vehicle.get_location()) < self.mrconfig.bb_save_radius:
        ...
        for static in static_list:
            if static.get_location().distance(self._vehicle.get_location()) < self.mrconfig.bb_save_radius:
        ...
        for stop_sign in stop_signs:
            if stop_sign.get_location().distance(self._vehicle.get_location()) < self.mrconfig.bb_save_radius:
    ...
    def lidar_to_histogram_features(self, lidar, use_ground_plane=False):
        ...
        def splat_points(point_cloud):
            xbins = np.linspace(self.mrconfig.min_x, self.mrconfig.max_x,
                                (self.mrconfig.max_x - self.mrconfig.min_x) * int(self.mrconfig.pixels_per_meter) + 1)
            ybins = np.linspace(self.mrconfig.min_y, self.mrconfig.max_y,
                                (self.mrconfig.max_y - self.mrconfig.min_y) * int(self.mrconfig.pixels_per_meter) + 1)
            hist = np.histogramdd(point_cloud[:, :2], bins=(xbins, ybins))[0]
            hist[hist > self.mrconfig.hist_max_per_pixel] = self.mrconfig.hist_max_per_pixel
            overhead_splat = hist / self.mrconfig.hist_max_per_pixel
        ...
        lidar = lidar[lidar[..., 2] < self.mrconfig.max_height_lidar]
        below = lidar[lidar[..., 2] <= self.mrconfig.lidar_split_height]
        above = lidar[lidar[..., 2] > self.mrconfig.lidar_split_height]
    ...
    def forecast_ego_agent(self, current_ego_transform, current_ego_speed, num_future_frames, ego_control):
        ...
        for _ in range(num_future_frames):
            ...
            if speed.item() < self.mrconfig.extent_ego_bbs_speed_threshold:
                extent.x *= self.mrconfig.slow_speed_extent_factor_ego
                extent.y *= self.mrconfig.slow_speed_extent_factor_ego
            else:
                extent.x *= self.mrconfig.high_speed_extent_factor_ego_x
                extent.y *= self.mrconfig.high_speed_extent_factor_ego_y
    ...
    def predict_other_actors_bounding_boxes_from_bev(self, pred_bb, ego_matrix, ego_yaw, num_future_frames,
        ...
        detection_radius = self.mrconfig.detection_radius
        ...
        for i, box in enumerate(pred_bb):
            ...
            if int(cls) not in self.mrconfig.Other_ACTORS_CLASSES:
                continue
        ...
        for actor_idx, actor_id in enumerate(nearby_actors.keys()):
            ...
            for t in range(num_future_frames):
                ...
                s = self.mrconfig.high_speed_min_extent_x_other_vehicle_lane_change if near_lane_change \
                    else self.mrconfig.high_speed_min_extent_x_other_vehicle
                extent.x *= self.mrconfig.slow_speed_extent_factor_ego if speed < self.mrconfig.extent_other_vehicles_bbs_speed_threshold else max(
                    s,
                    self.mrconfig.high_speed_min_extent_x_other_vehicle * float(t) / float(num_future_frames))

                extent.y *= self.mrconfig.slow_speed_extent_factor_ego if speed < self.mrconfig.extent_other_vehicles_bbs_speed_threshold else max(
                    self.mrconfig.high_speed_min_extent_y_other_vehicle,
                    self.mrconfig.high_speed_extent_y_factor_other_vehicle * float(t) / float(num_future_frames))
    ...
    def forecast_walkers(self, pred_bb, ego_matrix, ego_yaw, num_future_frames):
        ...
        for i, box in enumerate(pred_bb):
            ...
            pedestrian_speed = max(
                forward_speed, self.mrconfig.min_walker_speed)  # 0.5m/s
            ...
            for t in range(num_future_frames):
                future_pedestrian_locations[
                    t] = world_pos + t * pedestrian_direction * pedestrian_speed / self.mrconfig.bicycle_frame_rate

            # Create bounding boxes for future pedestrian positions
            pedestrian_future_bboxes = []
            extent = carla.Vector3D(w, h, 0.745139)
            extent.x = max(self.mrconfig.pedestrian_minimum_extent,
                           extent.x)  # Ensure a minimum width
            extent.y = max(self.mrconfig.pedestrian_minimum_extent, extent.y)
            for t in range(num_future_frames):
    ...
    def construct_stop_sign_bboxes(self, bounding_boxes, ego_matrix, ego_yaw, ego_speed):
        ...
        for bb in stop_sign_bboxes:
            ...
            if not state["cleared"] and distance < self.mrconfig.unclearing_distance_to_stop_sign:
    ...
    def kbm_based_collision_risk(self, pred_wp, pred_bb, control):
        ...
        num_future_frames = int(
            self.mrconfig.bicycle_frame_rate *
            (self.mrconfig.forecast_length_lane_change if near_lane_change else self.mrconfig.default_forecast_length))
        ...
        if collisions:
            ...
            collision_time = collision["frame_idx"] / self.mrconfig.time_step_inv
    ...
    def is_near_lane_change_idm(self, ego_velocity, route_points):
        ...
        braking_distance = ((
            (
                ego_velocity * 3.6) / 10.0) ** 2 / 2.0) + self.mrconfig.braking_distance_calculation_safety_distance

        # Determine the number of waypoints to look ahead based on the braking distance
        look_ahead_points = max(self.mrconfig.minimum_lookahead_distance_to_compute_near_lane_change,
                                min(route_points.shape[0], self.mrconfig.points_per_meter * int(braking_distance)))
        ...
        from_index = max(0, current_route_index -
                         self.mrconfig.check_previous_distance_for_lane_change)
    ...
    def _compute_target_speed_idm(self,
        ...
        a = self.mrconfig.idm_maximum_acceleration  # Maximum acceleration [m/s²]
        b = self.mrconfig.idm_comfortable_braking_deceleration_high_speed if ego_speed > \
            self.mrconfig.idm_comfortable_braking_deceleration_threshold else \
            self.mrconfig.idm_comfortable_braking_deceleration_low_speed  # Comfortable deceleration [m/s²]
        delta = self.mrconfig.idm_acceleration_exponent  # Acceleration exponent

        t_bound = self.mrconfig.idm_t_bound
    ...
    def _get_steer(self, route_points, current_position, current_heading, current_speed):
        """
            ...
            """
        speed_scale = self.mrconfig.lateral_pid_speed_scale
        speed_offset = self.mrconfig.lateral_pid_speed_offset
        ...
        lookahead_distance = np.clip(lookahead_distance, self.mrconfig.lateral_pid_default_lookahead,
                                     self.mrconfig.lateral_pid_maximum_lookahead_distance)
        lookahead_index = int(
    ...
    def get_brake_and_target_speed(self, route_points, distance_to_next_traffic_light, next_traffic_light,
        ...
        for ob in static_obstacles:
            if ob.get_location().distance(self._vehicle.get_location()) < self.mrconfig.static_obstacle_radius:
    ...
    def ego_agent_affected_by_red_light_idm(self, ego_vehicle_location, ego_vehicle_speed, distance_to_traffic_light,
        ...
        for light, center, waypoints in self.list_traffic_lights:
            ...
            if center_loc.distance(ego_vehicle_location) > self.mrconfig.light_radius:
                continue
        ...
        target_speed = self._compute_target_speed_idm(desired_speed=target_speed,
                                                      ...
                                                      s0=self.mrconfig.idm_red_light_minimum_distance,
                                                      T=self.mrconfig.idm_red_light_desired_time_headway)
    ...
    def ego_agent_affected_by_stop_sign_idm(self, ego_vehicle_location, ego_vehicle_speed, next_stop_sign, target_speed,
        ...
        if distance_to_stop_sign > self.mrconfig.unclearing_distance_to_stop_sign:
            self.cleared_stop_sign = False
            self.waiting_ticks_at_stop_sign = 0
        else:
            # Set the stop sign flag if we are closer than 3m and speed is low enough
            if ego_vehicle_speed < 0.1 and distance_to_stop_sign < self.mrconfig.clearing_distance_to_stop_sign:
        ...
        target_speed = self._compute_target_speed_idm(desired_speed=target_speed,
                                                      ...
                                                      s0=self.mrconfig.idm_stop_sign_minimum_distance,
                                                      T=self.mrconfig.idm_stop_sign_desired_time_headway)
            ...
            if vehicle_id in leading_vehicle_ids and not near_lane_change:
                ...
                target_speed_wrt_leading_vehicle = min(
                    ...
                    self._compute_target_speed_idm(desired_speed=initial_target_speed,
                                                   ...
                                                   s0=self.mrconfig.idm_leading_vehicle_minimum_distance,
                                                   T=self.mrconfig.idm_leading_vehicle_time_headway))
    ...
    def compute_target_speeds_wrt_all_actors_idm(self, initial_target_speed, ego_bounding_boxes,
        ...
        ego_vehicle_location = self._vehicle.get_location()
        hazard_color = self.mrconfig.ego_vehicle_forecasted_bbs_hazard_color
        normal_color = self.mrconfig.ego_vehicle_forecasted_bbs_normal_color
        color = normal_color
                        ...
                        if "base_type" in blocking_actor.attributes and blocking_actor.attributes[
                            ...
                            target_speed_bicycle = min(
                                ...
                                self._compute_target_speed_idm(desired_speed=initial_target_speed,
                                                               ...
                                                               s0=self.mrconfig.idm_bicycle_minimum_distance,
                                                               T=self.mrconfig.idm_bicycle_desired_time_headway))
                ...
                if t_u.check_obb_intersection(ego_bounding_box, pedestrian_bb[i]):
                    ...
                    target_speed_pedestrian = min(
                        ...
                        self._compute_target_speed_idm(desired_speed=initial_target_speed,
                                                       ...
                                                       leading_actor_speed=0.,
                                                       distance_to_leading_actor=distance_to_actor,
                                                       s0=self.mrconfig.idm_pedestrian_minimum_distance,
                                                       T=self.mrconfig.idm_pedestrian_desired_time_headway))
                ...
                if t_u.check_obb_intersection(ego_bounding_box, static_bb):
                    ...
                    target_speed_static = min(
                        ...
                        self._compute_target_speed_idm(
                            ...
                            s0=self.mrconfig.idm_static_minimum_distance,
                            T=self.mrconfig.idm_static_desired_time_headway
                        )
                    )
    ...
    def forecast_ego_agent_idm(self, current_ego_transform, current_ego_speed, num_future_frames, initial_target_speed,
        ...
        for _ in range(num_future_frames):
            ...
            extent.x *= self.mrconfig.slow_speed_extent_factor_ego if current_ego_speed < \
                self.mrconfig.extent_ego_bbs_speed_threshold else self.mrconfig.high_speed_extent_factor_ego_x
            extent.y *= self.mrconfig.slow_speed_extent_factor_ego if current_ego_speed < \
                self.mrconfig.extent_ego_bbs_speed_threshold else self.mrconfig.high_speed_extent_factor_ego_y
    ...
    def forecast_walkers_idm(self, actors, ego_vehicle_location, number_of_future_frames):
        ...
        pedestrians = [
            ...
            if ped.get_location().distance(ego_vehicle_location) < self.mrconfig.detection_radius
        ]
        ...
        pedestrian_speeds = np.maximum(
            pedestrian_speeds, self.mrconfig.min_walker_speed)
        ...
        future_pedestrian_locations = pedestrian_locations[:, None, :] + np.arange(1, number_of_future_frames + 1)[
            None, :, None] * pedestrian_directions[:, None,
                                                   :] * pedestrian_speeds[:,
                                                                          None,
                                                                          None] / self.mrconfig.bicycle_frame_rate
        ...
        for i, ped in enumerate(pedestrians):
            ...
            extent.x = max(self.mrconfig.pedestrian_minimum_extent,
                           extent.x)  # Ensure a minimum width
            extent.y = max(self.mrconfig.pedestrian_minimum_extent,
    ...
    def predict_other_actors_bounding_boxes_idm(self, actor_list, ego_vehicle_location, num_future_frames,
        ...
        nearby_actors = [
            ...
            actor.get_location().distance(
                ego_vehicle_location) < self.mrconfig.detection_radius
        ]
        ...
        if nearby_actors:
            ...
            for actor_idx, actor in enumerate(nearby_actors):
                ...
                for i in range(num_future_frames):
                    ...
                    extent.x *= self.mrconfig.slow_speed_extent_factor_ego if future_velocities[
                        i, actor_idx] < self.mrconfig.extent_other_vehicles_bbs_speed_threshold else max(
                        s,
                        self.mrconfig.high_speed_min_extent_x_other_vehicle * float(i) / float(num_future_frames))
                    extent.y *= self.mrconfig.slow_speed_extent_factor_ego if future_velocities[
                        i, actor_idx] < self.mrconfig.extent_other_vehicles_bbs_speed_threshold else max(
                        self.mrconfig.high_speed_min_extent_y_other_vehicle,
                        self.mrconfig.high_speed_extent_y_factor_other_vehicle * float(i) / float(num_future_frames))
    ...
    def _get_control_idm(self, ego_position):
        ...
        control.steer = steer + self.mrconfig.steer_noise * np.random.randn()
        control.throttle = throttle
        control.brake = float(brake or control_brake)

        # Apply brake if the vehicle is stopped to prevent rolling back
        if control.throttle == 0 and ego_speed < self.mrconfig.minimum_speed_to_prevent_rolling_back:
            control.brake = 1
