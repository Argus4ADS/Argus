...
def merge_route_json(folder_path):
    ...
    if len(merged_records) != 10:
        print(f"-----------------------Warning: there are {len(merged_records)} routes in your json, which does not equal to 10. All metrics (Driving Score, Success Rate, Ability) are inaccurate!!!")
    ...
    merged_data = {
        ...
        "driving score": sum(driving_score) / 10,
        "success rate": success_num / 10,
        "route complete": sum(route_complete) / 10,
