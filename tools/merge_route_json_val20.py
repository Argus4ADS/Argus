...
def merge_route_json(folder_path):
    ...
    if len(merged_records) != 20:
        print(f"-----------------------Warning: there are {len(merged_records)} routes in your json, which does not equal to 20. All metrics (Driving Score, Success Rate, Ability) are inaccurate!!!")
    ...
    merged_data = {
        ...
        "route complete": sum(route_complete) / 20,
